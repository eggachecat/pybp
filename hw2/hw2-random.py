...
SQLiteDB = "random_exp_records.db"
...
def test(nnConfig, epoch, maxRandomSelection = 0):


	sizeOfLayers = len(nnConfig["layers"])

	uuid = str(np.random.randint(0, 1000000000))
	recordDir = os.path.join(os.path.dirname(__file__ ) + "\\random_exp_figures\\")
	...
	for trainLayerIndex in range(1, sizeOfLayers):
		...
		for x in range(0, epoch):
			diffObj = NN.train(dataSet, trainLayerIndex, fractionCtr < maxRandomSelection)
	...
	exp_note = uuid
	exp_id = nnSQLite.saveToGeneralDB(NN.toDict(), ini_nn_dict, initialErrorRate, errorRate, epoch, "random-classification", dataset_name, exp_note, uuid)
